...
export const PROJECTS: Project[] = [
  {
    ...
    problem:
      'Every AI assistant is stateless and reactive. You re-explain your project every session, you have to summon it manually, and each one locks you into a single model with a single price. Nothing watches the work you’re actually doing.',
    solution:
      'A desktop-native brain that persists. AURA keeps long-term memory across sessions, indexes your projects, routes each request to whichever of 8 models fits best across 2 providers, and speaks up on its own when it notices something worth saying — through a voice interface you never have to click.',
    architecture: [
    ...
    features: [
      [
        ...
        'automatic model selection across 8 models and 2 providers, with fallback, prompt optimisation and context-aware responses.',
      ],
      [
        'Voice pipeline',
        'wake word, speech-to-text, text-to-speech and continuous conversation without touching the keyboard.',
      ],
      [
        'Memory system',
        'long-term persistent memory, retrieval and full project indexing — it recalls instead of restarting.',
      ],
      [
        'Intelligence layer',
        'Curiosity Engine, Attention Engine, Personality Layer and a Proactive Assistant that stays quiet unless it helps.',
      ],
      [
        'Productivity surface',
        'tasks, notes, research and a built-in Workbench. Calendar planned.',
      ],
      [
        'Cosmic UI',
        'black-hole orb, white-hole speaking animation, the Great Attractor, planet-based model selection, memory timeline and cosmic web.',
      ],
    ],
    stack: {
...

  {
    ...
    problem:
      'Remote learning flattened education into a grid of muted video tiles. There is no hallway, no walking into a club, no sitting down next to someone — the accidental social layer that makes a campus work simply disappeared.',
    solution:
      'Rebuild the campus as a shared 3D space. Students move, gather and talk in real time inside buildings that map to real activities — classrooms, clubs, events — with an AI assistant that suggests collaborations rather than policing attendance.',
    architecture: [
      'Next.js Client',
      'Three.js Campus',
      'WebSocket Server',
    ...
    features: [
      [
        'Multiplayer core',
        'real-time movement, player synchronisation, voice chat and live communication.',
      ],
      ...
      [
        'Research angle',
        'educational metaverse, AI collaboration, smart learning and social interaction.',
      ],
    ],
    stack: {
    ...
    metrics: [],
    timeline: [
      ['Stage 1', 'Campus world design and movement prototype.'],
      ['Stage 2', 'Multiplayer sync and voice channels.'],
      ['Stage 3', 'AI assistant and collaboration engine.'],
      ['Stage 4', 'Research write-up.'],
    ],
    challenges: [
    ...
    future: [
      'Class scheduling inside the world',
      'Persistent student profiles',
      'Cross-institution campuses',
...

  {
    ...
    overview:
      'A real-time 3D representation of a smart city that visualises infrastructure, traffic, environmental conditions and public services while integrating AI-powered analytics and IoT data.',
    problem:
      'City data already exists — traffic counters, air quality sensors, grid load, weather stations — but it lives in disconnected dashboards nobody reads together. There is no single place where you can see the city as one system.',
    solution:
      'A digital twin: one 3D model of the city where every live feed is rendered in place. Traffic flows on the actual roads, pollution shades the actual districts, and an AI layer turns that combined picture into predictions, recommendations and early disaster alerts.',
    architecture: [
      'IoT Sensors',
      'Ingestion Layer',
      'PostgreSQL + GIS',
      'Python Analytics',
      'Next.js + Three.js Twin',
    ],
    features: [
    ...
    stack: {
      ...
      Backend: ['Python', 'GIS APIs'],
      Data: ['PostgreSQL'],
    },
    metrics: [],
    timeline: [
      ['Stage 1', 'City geometry and GIS import.'],
      ['Stage 2', 'Live data ingestion and dashboards.'],
      ['Stage 3', 'Predictive models and alerting.'],
      ['Stage 4', 'Simulation tooling.'],
    ],
    challenges: [
...

  {
    ...
    overview:
      'A desktop file management application focused on efficient file organisation and duplicate detection. It applies operating system concepts through practical file management utilities.',
    problem:
      'Storage fills up invisibly. The duplicates, the forgotten downloads, the folders you copied "just in case" — no native tool tells you where the weight actually is, and the ones that do are bloated and cloud-tethered.',
    solution:
      'A fast, native, dependency-light desktop app in C++ and Qt that scans directories quickly, finds true duplicates, and applies real OS caching algorithms — LRU and SRU — to file management rather than leaving them in a textbook.',
    architecture: [
      'Qt GUI',
      'Scan Engine',
      'Duplicate Detector',
    ...
    metrics: [],
    timeline: [
      ['Build', 'Scan engine and duplicate detection.'],
      ['Build', 'LRU / SRU implementation.'],
      ['Ship', 'Native Qt interface, completed.'],
    ],
    challenges: [
...

  {
    ...
    overview:
      'A complete redesign of WasabiKiri with a Japanese-inspired interface, improved performance and additional storage management capabilities. Design theme: samurai, torii gates, sakura, traditional Japanese architecture and colour palette.',
    problem:
      'The original proved the engine but looked like a utility. It also stopped at detection — it could tell you what was duplicated, but not what your storage actually looked like or what to do next.',
    solution:
      'Rebuild the whole surface. A calm, deliberately Japanese interface, automatic scanning that runs without being asked, category-level analytics with real charts, and cleanup actions that go beyond duplicates to old files, large files and empty folders.',
    architecture: [
      'Qt GUI (redesigned)',
      'Auto-Scan Scheduler',
      'Cleanup Engine',
    ...
    features: [
      [
        'Storage engine',
        'automatic scanning, duplicate cleanup, old-file, large-file and empty-folder detection.',
      ],
      [
        'Analytics',
        'storage visualisation, file-category statistics and interactive charts.',
      ],
    ...
    metrics: [],
    timeline: [
      ['Stage 1', 'Visual language and design system.'],
      ['Stage 2', 'Cleanup engine expansion.'],
      ['Stage 3', 'Analytics and charts.'],
...

  {
    ...
    overview:
      'A web application that recommends tourist destinations using machine learning and user preferences. The project demonstrates AI-assisted recommendation systems deployed through a modern web stack.',
    problem:
      'Travel search gives you the most popular result, not the right one. Preference — budget, pace, terrain, season — gets flattened into a ranked list that is identical for everybody.',
    solution:
      'A recommendation model that takes stated preferences as input and returns destinations matched to them, served live through a Next.js interface and deployed publicly rather than left in a notebook.',
    architecture: [
      'Next.js Client',
      'Preference Input',
      'Python ML Inference',
    ...
    metrics: [],
    timeline: [
      ['Stage 1', 'Dataset and model.'],
      ['Stage 2', 'Web interface.'],
      ['Stage 3', 'Deployment on Vercel.'],
    ],
    challenges: [
...

  {
    ...
    overview:
      'A machine learning project that forecasts tourist arrivals using historical and seasonal data. The system is intended to support tourism planning and resource allocation.',
    problem:
      'Tourism infrastructure is staffed and stocked on instinct. Under-prepare and the season overwhelms you; over-prepare and the cost sits idle — and the historical data that would answer it is rarely modelled.',
    solution:
      'A complete forecasting pipeline over historical arrival data, with seasonality made explicit as a feature, producing arrival predictions and clear visualisations planners can actually act on.',
    architecture: [
      'Data Collection',
      'Cleaning',
      'Feature Engineering',
...

  {
    ...
    overview:
      'A fraud detection platform that analyses financial transactions using machine learning to identify suspicious activity and assign fraud risk scores.',
    problem:
      'Fraud is rare, which is exactly what makes it hard — a model that flags nothing is right 99% of the time. Rule-based systems miss new patterns and drown analysts in false positives.',
    solution:
      'A trained classifier over engineered transaction features that outputs a risk score rather than a binary verdict, surfaced in a dashboard so an analyst can triage by severity instead of reading every row.',
    architecture: [
      'Transaction Data',
      'Preprocessing',
      'Feature Engineering',
    ...
    stack: {
      Language: ['Python'],
      ML: ['Scikit-learn', 'Pandas', 'NumPy'],
      App: ['Flask', 'HTML', 'CSS', 'JavaScript'],
    },
    metrics: [],
    timeline: [
      ['Stage 1', 'Dataset preprocessing.'],
      ['Stage 2', 'Feature engineering.'],
      ['Stage 3', 'Model training and fraud classification.'],
      ['Stage 4', 'Evaluation and dashboard.'],
    ],
    challenges: [
